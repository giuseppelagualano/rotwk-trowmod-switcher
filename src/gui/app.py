...
from core.config import (
    REPO_OWNER, REPO_NAME, REGISTRY_PATHS_ROTWK, __APP_VERSION__,
    APPDATA_FOLDER, CONFIG_FILE_NAME, CONFIG_PATH_SECTION,
    LOCAL_CONTENT_KEY, ROTWK_CONTENT_KEY, __APP_NAME__ # Import app name if used in paths/messages
)
...
from .theme import (
    TEXT_PRIMARY, TEXT_SECONDARY, BUTTON_PRIMARY_BG, BUTTON_SECONDARY_BG,
    BUTTON_PRIMARY_BORDER, BUTTON_PRIMARY_HOVER, BUTTON_SECONDARY_HOVER,
    BUTTON_TEXT_PRIMARY, BUTTON_TEXT_SECONDARY, BUTTON_TERTIARY_BG,
    PRIMARY_BUTTON_FONT, SECONDARY_BUTTON_FONT, TERTIARY_BUTTON_FONT,
    LABEL_FONT, FLAG_FONT, TEXT_FONT, INITIAL_WINDOW_SIZE, APP_TITLE
)
...
def update_flag(success):
    ...
    if not flag_label: return # Guard against missing widget
    if success:
...
def set_buttons_state(new_state):
    ...
    widgets = [remote_update_button, local_update_button, launch_game_button,
               browse_button_remote, browse_button_local]
...
def perform_update_check(show_no_update_message=False):
    ...
    repo_full_name = f"{REPO_OWNER}/{REPO_NAME}" # Define repo name used by updater
    ...
    def check_thread_target():
        ...
        try:
            ...
            is_update, latest_v, url = check_for_updates() # Uses UPDATER_GITHUB_REPO from config
...
def on_remote_update_click():
    ...
    if not rotwk_path_entry or not flag_label: return
...
def on_local_update_click():
    ...
    if not rotwk_path_entry or not local_path_entry or not flag_label: return
...
def browse_rotwk_path():
    ...
    if not rotwk_path_entry: return
    ...
    if directory:
        ...
        save_config(APPDATA_FOLDER + CONFIG_FILE_NAME, CONFIG_PATH_SECTION, ROTWK_CONTENT_KEY, normalized_path)

def browse_local_dev_path():
    """Opens a dialog to browse for the local DEV mod folder path."""
    if not local_path_entry: return
    ...
    if directory:
        ...
        save_config(APPDATA_FOLDER + CONFIG_FILE_NAME, CONFIG_PATH_SECTION, LOCAL_CONTENT_KEY, normalized_path)
    ...
    def emit(self, record):
        ...
        if log_console: # Check if log_console has been initialized
            try:
                ...
                print(f"Error updating log console: {e}\nLog message: {msg}", file=sys.stderr) # Fallback print

    def _update_log_console(self, msg):
         if not log_console or not log_console.winfo_exists(): return # Extra safety check
...
def run_gui():
    ...
    browse_button_remote = ctk.CTkButton(
        remote_frame, text="Browse...", font=TERTIARY_BUTTON_FONT, command=browse_rotwk_path, width=80,
        fg_color=BUTTON_TERTIARY_BG, hover_color=BUTTON_PRIMARY_HOVER, border_color=BUTTON_PRIMARY_BORDER, border_width=1
    )
    browse_button_remote.grid(row=0, column=1, padx=5, pady=10)

    remote_update_button = ctk.CTkButton(
        remote_frame, text="Online Update", font=PRIMARY_BUTTON_FONT, text_color=TEXT_PRIMARY, command=on_remote_update_click,
    ...
    loaded_local_path = load_config(APPDATA_FOLDER + CONFIG_FILE_NAME, CONFIG_PATH_SECTION, LOCAL_CONTENT_KEY, "Insert DEV Mod folder path here")
    local_path_entry.insert(0, loaded_local_path)

    browse_button_local = ctk.CTkButton(
        local_frame, text="Browse...", font=TERTIARY_BUTTON_FONT, command=browse_local_dev_path, width=80,
        fg_color=BUTTON_TERTIARY_BG, hover_color=BUTTON_PRIMARY_HOVER, border_color=BUTTON_SECONDARY_HOVER, border_width=1
    )
    browse_button_local.grid(row=0, column=1, padx=5, pady=10)

    local_update_button = ctk.CTkButton(
        local_frame, text="Local Update", font=SECONDARY_BUTTON_FONT, text_color=BUTTON_TEXT_SECONDARY, command=on_local_update_click,
        fg_color=BUTTON_SECONDARY_BG, hover_color=BUTTON_PRIMARY_HOVER, border_color=BUTTON_SECONDARY_HOVER, border_width=2
    )
    ...
    launch_game_button = ctk.CTkButton(
        flag_frame, text="Launch Game", image=game_ico, font=SECONDARY_BUTTON_FONT, command=on_launch_game_click,
        fg_color="#1c2c2c", hover_color=BUTTON_PRIMARY_HOVER, border_color=BUTTON_SECONDARY_HOVER, border_width=1, width=120
    )
